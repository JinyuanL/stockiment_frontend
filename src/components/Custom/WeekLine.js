...
function parseData(data) {
    const locationOfFirst = data.length - 5;
    var pricesList = [];
    var volumeList = [];
    var sentimentList = [];
    var highestDollar = 0;
    ...
    for (var i = locationOfFirst; i < data.length; i++) {
        ...
        // low chart
        const low = {};
        low.x = `${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate() +1}`;
        low.y = data[i].Low;
        if (low.y != null)
        ...
        // close chart
        const close = {};
        close.x = `${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate() + 1}`;
        ...
        // high chart
        const high = {};
        high.x = `${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate() + 1}`;
        high.y = data[i].High;
        ...
        // volume chart
        const volume = {};
        volume.x = `${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate() + 1}`;
        ...
        // sentiment chart
        const sentiment = {};
        sentiment.x = `${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate() + 1}`;
    ...
    const oldPrediction = {};
    oldPrediction.x = `${lastDate.getFullYear()}-${lastDate.getMonth() + 1}-${lastDate.getDate() + 1}`;
    oldPrediction.y = lastClose;
    pricesList[3].data.push(oldPrediction);

    const prediction = {};
    const sentiment = {};
    prediction.x = `${lastDate.getFullYear()}-${lastDate.getMonth() + 1}-${lastDate.getDate() + 2}`;
    ...
    sentiment.x = `${lastDate.getFullYear()}-${lastDate.getMonth() + 1}-${lastDate.getDate() + 2}`;
    sentiment.y = null;
...
export class WeekLine extends React.Component {
    constructor(props) {
        super(props);
